function [feature_table] = extractFeaturesExample(dataChTimeTr,includedFeatures, Fs)
    ...
    for f = 1:length(includedFeatures)
        ...
        switch includedFeatures{f}
            ...
            case 'rel_var'
               fvalues = (squeeze(var(dataChTimeTr(:,1:700,:),0,2)) ./ squeeze(var(dataChTimeTr(:,700:1400,:),0,2)))';

            case 'mean_freq'
                fvalues = [];
                for i = 1:4
                    res = squeeze(meanfreq(squeeze(dataChTimeTr(i,1:1400,:))))';
                    fvalues(:,i) =+ res;
                end
            ...
            case "rel_mean_freq"
                fvalues = [];
                for i = 1:4
                    res = (squeeze(meanfreq(squeeze(dataChTimeTr(3,1:700,:)))) ./ squeeze(meanfreq(squeeze(dataChTimeTr(3,700:1400,:)))))';
                    fvalues(:,i) =+ res;
                end
